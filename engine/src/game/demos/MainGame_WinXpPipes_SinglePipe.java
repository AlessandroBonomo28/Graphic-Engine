...
public class MainGame_WinXpPipes_SinglePipe extends GameEngine
{
	...
	Vector3 topRightBound = Vector3.one().multiply(10);
	...
	public MainGame_WinXpPipes_SinglePipe(JFrame window,int widthWindow,int heightWindow) throws MatrixException {
...
//			camera.transla(Vector3.down().multiply(0.5f), Space.World);
			spawnPipe();
//			pipeTransform = new Transform(objReader.readMeshFromFile(pipeFaceToExtrudeFilePath),new Vector3(0,0,0)); 
//			pipeTransform.transla(Vector3.forward().multiply(5), Space.World);
//			
//			currentPipeMesh = pipeTransform.getMesh();
//			
...
//			pipeFaceToExtrude = extrudeFace(currentPipeMesh.getTriangles(), currentPipeMesh, dir, euler);
//			savePositionIntoMap(getCurrentPos());
	...
	public static void main(String[] args) 
	{
		try {
			// start game
			new MainGame_WinXpPipes_SinglePipe(new JFrame(), WIDTH,HEIGHT).start();
		}
		catch (MatrixException e){
			e.printStackTrace();
		}
	...
	protected void update() 
	{
		...
		if(timerReset>= tempoReset)
		{
			...
			try {
				...
				System.out.println("Respawn pipe at position "+getCurrentPos()+
						".. next respawn in "+tempoReset+ " sec");
			} catch (FileNotFoundException e) {
		...
		try {
			if(distCovered>=distToTravel) // ha raggiunto la destinazione impostata
			{
				...
				if(res == 0 && !isSpawning) // svolta con step
				{
